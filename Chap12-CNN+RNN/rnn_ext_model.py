# -*- coding: utf-8 -*-
"""
Created on Thu Feb 13 10:54:03 2020

@author: Jinsung
"""

import sys
sys.path.insert(0, 'C:\\Users\\Jinsung\\Documents\\Deep_Learning_Code\\Chap11-LSTM')
from rnn_lstm_model import *

class RnnExtModel(RnnLstmModel):
    pass

# 시계열 포장 계층에 대한 파라미터 생성 메서드 정의 
...
# 출력 계층에서의 시계열 데이터 처리를 위한 파라미터 생성 메서드 재정의
def rnn_ext_init_parameters(self, hconfigs):
    ...
    self.pm_out, _ = self.alloc_layer_param(prev_shape, output_cnt)
    
RnnExtModel.init_parameters = rnn_ext_init_parameters

# 출력 계층에서의 시계열 데이터 처리를 위한 순전파 메서드 재정의
def rnn_ext_forward_neuralnet(self, x):
    ...
    if self.seqout:
        ...
        out_flat, aux_flat = self.forward_layer(hidden_flat, None, self.pm_output)
...
# 출력 계층에서의 시계열 데이터 처리를 위한 역전파 메서드 재정의
def rnn_ext_backprop_neuralnet(self, G_output, aux):
    ...
    if self.seqout:
        ...
        G_hidden_flat = self.backprop_layer(G_out_flat, None, self.pm_output, aux_flat)
