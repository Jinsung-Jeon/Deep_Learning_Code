# -*- coding: utf-8 -*-
"""
Created on Wed Feb 12 10:41:12 2020

@author: Jinsung
"""

import sys
sys.path.insert(0, 'C:\\Users\\Jinsung\\Documents\\Deep_Learning_Code\\Chap10-Simple RNN')
from rnn_basic_model import *

class RnnLstmModel(RnnBasicModel):
    pass

# LSTM 계층에 대한 파라미터 생성 메서드 정의하기
...
def rnn_backprop_lstm_layer(self, G_y, hconfig, pm, aux):
    ...
    if outseq:
        ...
        for n in reversed(range(0, max_length)):
            ...
            G_affine[:, 0*recur_size:1*recur_size] = sigmoid_derv(forget_gate) * G_forget_gate
            G_affine[:, 1*recur_size:2*recur_size] = sigmoid_derv(input_gate) * G_input_gate
            G_affine[:, 2*recur_size:3*recur_size] = sigmoid_derv(output_gate) * G_output_gate
            G_affine[:, 3*recur_size:3*recur_size] = tanh_derv(block_input) * G_block_input
